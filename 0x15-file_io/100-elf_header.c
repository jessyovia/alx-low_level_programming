...
/**
* verify - Verify if the file is an ELF
* @e_ident: The elf_header elf_filename
*  Description: Compile this program using gcc and provide the ELF
*  file as an argument. It will display the ELF header information
*  similar to the readelf -h command.
*/
...
*/
void type(int e_type, char *e_ident)
{
...
*/
void entry(int e_entry, char *e_ident)
{
...
int main(int argc, char *argv[])
{
	...
	f_read = read(fileD, file, sizeof(Elf64_Ehdr));
	if (f_read == -1)
	{
		free(file);
		dprintf(STDERR_FILENO, "Error: Can't read from file %s\n", argv[1]);
